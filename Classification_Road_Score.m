...
if contains(root_dir, 'sturbois_chipseal_woods_1')
    ...
    load('sturbois_chipseal_woods_1_MANUAL_CLASSIFICATION.mat');
...
elseif contains(root_dir, 'sturbois_chipseal_woods_2')
    ...
    load('sturbois_chipseal_woods_2_MANUAL_CLASSIFICATION.mat');
...
elseif contains(root_dir, '2022-10-11-09-24-00')
    ...
    load('2022-10-11-09-24-00_MANUAL_CLASSIFICATION.mat');
...
elseif contains(root_dir, '2022-10-20-10-14-05_GRAV')
    ...
    load('2022-10-20-10-14-05_GRAV_MANUAL_CLASSIFICATION.mat');
...
elseif contains(root_dir, 'sturbois_curve_1')
    ...
    load('sturbois_curve_1_MANUAL_CLASSIFICATION.mat');
...
elseif contains(root_dir, 'sturbois_straight_1')
    ...
    load('sturbois_straight_1_MANUAL_CLASSIFICATION.mat');
...
% try
    ...
    if chip_bool
        ...
        for i = 1:length(Manual_Classfied_Areas.chip)
            ...
            plot(pgon,'FaceColor','white','FaceAlpha',0.25)
...
if non_road_bool
    ...
    for i = 1:1:length(Manual_Classfied_Areas.non_road_roi)
        ...
        % Getting all of the used points (some points lie outside defined areas
        % - unscoreable therefore those points are removed).
        grav_offroad_array       = [grav_offroad_array; find(grav_in_gras == 1)];
        chip_offroad_array       = [chip_offroad_array; find(chip_in_gras == 1)];
        foli_offroad_array       = [foli_offroad_array; find(foli_in_gras == 1)];
        gras_offroad_array       = [gras_offroad_array; find(gras_in_gras == 1)];
...
if non_road_bool
    ...
    num_used_IN_nonroad_area = grav_in_foli_count + chip_in_foli_count + foli_in_foli_count + gras_in_foli_count;
    
    num_used_nonroad = foli_in_grav_count + foli_in_chip_count + foli_in_foli_count + foli_in_gras_count;
    
    grav_in_nonroad_score = grav_in_foli_count / num_used_nonroad * 100;
    chip_in_nonroad_score = chip_in_foli_count / num_used_nonroad * 100;
    foli_in_nonroad_score = foli_in_foli_count / num_used_nonroad * 100;
    gras_in_nonroad_score = gras_in_foli_count / num_used_nonroad * 100;
...
if road_bool
    ...
    num_used_IN_road_area = grav_in_foli_count + chip_in_foli_count + foli_in_foli_count + gras_in_foli_count;
    
    num_used_nonroad = foli_in_grav_count + foli_in_chip_count + foli_in_foli_count + foli_in_gras_count;
    
    grav_in_road_score = grav_in_foli_count / num_used_nonroad * 100;
    chip_in_road_score = chip_in_foli_count / num_used_nonroad * 100;
    foli_in_road_score = foli_in_foli_count / num_used_nonroad * 100;
    gras_in_road_score = gras_in_foli_count / num_used_nonroad * 100;
...
score_offroad_array(1,2) = chip_in_nonroad_count / tot_num_non_road * 100;
score_offroad_array(1,3) = foli_in_nonroad_count / tot_num_non_road * 100;
score_offroad_array(1,4) = gras_in_nonroad_count / tot_num_non_road * 100;
...
grav_tag = ones(length(grav_array(grav_inroad_array,1)),1) * 1;
chip_tag = ones(length(chip_array(chip_inroad_array,1)),1) * 2;
foli_tag = ones(length(foli_array(foli_inroad_array,1)),1) * 3;
gras_tag = ones(length(gras_array(gras_inroad_array,1)),1) * 4;

% Making an array for each used item
grav_used_array = [grav_array(grav_inroad_array,1), grav_array(grav_inroad_array,2), grav_array(grav_inroad_array,3), grav_tag];
chip_used_array = [chip_array(chip_inroad_array,1), chip_array(chip_inroad_array,2), chip_array(chip_inroad_array,3), chip_tag];
foli_used_array = [foli_array(foli_inroad_array,1), foli_array(foli_inroad_array,2), foli_array(foli_inroad_array,3), foli_tag];
gras_used_array = [gras_array(gras_inroad_array,1), gras_array(gras_inroad_array,2), gras_array(gras_inroad_array,3), gras_tag];
...
if foli_bool
    ...
    for i = 1:1:length(Manual_Classfied_Areas.foli)
        ...
        % Getting all of the used points (some points lie outside defined areas
        % - unscoreable therefore those points are removed).
        grav_inroad_array       = [grav_used_idx; find(grav_in_foli == 1)];
        chip_inroad_array       = [chip_used_idx; find(chip_in_foli == 1)];
        foli_inroad_array       = [foli_used_idx; find(foli_in_foli == 1)];
        gras_inroad_array       = [gras_used_idx; find(gras_in_foli == 1)];
...
if gras_bool
    ...
    for i = 1:1:length(Manual_Classfied_Areas.gras)
        ...
        % Getting all of the used points (some points lie outside defined areas
        % - unscoreable therefore those points are removed).
        grav_inroad_array       = [grav_used_idx; find(grav_in_gras == 1)];
        chip_inroad_array       = [chip_used_idx; find(chip_in_gras == 1)];
        foli_inroad_array       = [foli_used_idx; find(foli_in_gras == 1)];
        gras_inroad_array       = [gras_used_idx; find(gras_in_gras == 1)];
...
plot3(grav_used_array_filt(grav_inroad_array,1), grav_used_array_filt(grav_inroad_array,2), grav_used_array_filt(grav_inroad_array,3), 'c.', 'MarkerSize', 18)
plot3(chip_used_array_filt(chip_inroad_array,1), chip_used_array_filt(chip_inroad_array,2), chip_used_array_filt(chip_inroad_array,3), 'k.', 'MarkerSize', 18)
plot3(foli_used_array_filt(foli_inroad_array,1), foli_used_array_filt(foli_inroad_array,2), foli_used_array_filt(foli_inroad_array,3), 'm.', 'MarkerSize', 18)
plot3(gras_used_array_filt(gras_inroad_array,1), gras_used_array_filt(gras_inroad_array,2), gras_used_array_filt(gras_inroad_array,3), 'g.', 'MarkerSize', 18)
