...
function table_export = gettoptwentyfeatsmls(xyzi, abcd)
    
    %% Table Header
    table_head = {    'Min2MaxRatioHeight',...
    ...
    data_data = [min(height)^2 / max(height),...
    (min(height) / max(height))/ xy_dist,...
    (min(height)^2 / max(height))/ xy_dist,...
    (min(height)^2 / max(height))/ range_mean,...
    min(range)^2 / max(range),...
    sqrt(sum(gradient(range).^2)),...
    sqrt(sum(gradient(range).^2)) / xy_dist,...
    sqrt(sum(gradient(range).^2)) / range_mean,...
    min(xyzi(:,3))^2 / max(xyzi(:,3)),...
    sqrt(sum(gradient(xyzi(:,3)).^2)),...
    (min(xyzi(:,3))^2 / max(xyzi(:,3))) / xy_dist,...
    (min(xyzi(:,3))^2 / max(xyzi(:,3))) / range_mean,...
    sqrt(sum(gradient(xyzi(:,3)).^2)) / range_mean,...
    std(xyzi(:,4)),...
    max(xyzi(:,4)) - min(xyzi(:,4)),...
    sqrt(sum(gradient(xyzi(:,4)).^2)),...
    std(xyzi(:,4)) / xy_dist,...
    (max(xyzi(:,4)) - min(xyzi(:,4))) / xy_dist,...
    sqrt(sum(gradient(xyzi(:,4)).^2)) / xy_dist,...
