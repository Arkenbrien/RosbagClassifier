...
function table_export = get_RANSAC_feats_2(xyzi, options)
    
    %% Table Header
    table_head   = {'StandDevHeightRAN',...
                    'RoughnessHeightRAN',...
                    'MinMaxRatioHeightRAN',...
                    'Min2MaxRatioHeightRAN',...
                    'MagGradientHeightRAN',...
                    'StandDevInt',...
                    'MeanInt',...
                    'RangeInt',...
                    'MedMaxRatioInt',...
    ...
    MaxNumTrials                = options.MaxNumTrials;
    ptCloudSource               = pointCloud([xyzi(:,1), xyzi(:,2), xyzi(:,3)]);
    model_RANSAC                = pcfitplane(ptCloudSource, maxDistance, 'MaxNumTrials', MaxNumTrials);
    ...
    xy_dist_mean                = mean(xy_dist);
    
    % Getting height Properties
    h_num                       = abs((abcd(1) * xyzi(:,1)) + (abcd(2) *  xyzi(:,2)) + (abcd(3) *  xyzi(:,3)) - abcd(4));
...
%     data_data = [std(height) / height_mean,...
%                 (max(height) - min(height)) / height_mean,...
%                 min(height) / max(height),...
%                 (min(height)^2 / max(height)) / height_mean,...
%                 sqrt(sum(gradient(height).^2)) / height_mean,...
%                 std(xyzi(:,4)),...
%                 mean(xyzi(:,4)),...
...
%                 sqrt(sum(gradient(xyzi(:,4)).^2))];
    ...
    data_data = [std(height) / xy_dist_mean,...
                (max(height) - min(height)) / xy_dist_mean,...
                min(height) / max(height),...
                (min(height)^2 / max(height)) / xy_dist_mean,...
                sqrt(sum(gradient(height).^2)) / xy_dist_mean,...
                std(xyzi(:,4)),...
                mean(xyzi(:,4)),...
                max(xyzi(:,4)) - min(xyzi(:,4)),...
                median(xyzi(:,4)) / max(xyzi(:,4)),...
