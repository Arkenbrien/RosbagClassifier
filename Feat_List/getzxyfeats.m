...
function table_export = getzxyfeats(xyzi)
    
    %% Table Header
    table_head = {'StandDevZ',...
    'RoughnessZ',...
    'MinMaxRatioZ',...
    'Min2MaxRatioZ',...
    'MagGradientZ',...
    ...
    data_data = [std(xyzi(:,3)),...
    max(xyzi(:,3)) - min(xyzi(:,3)),...
    min(xyzi(:,3)) / max(xyzi(:,3)),...
    min(xyzi(:,3))^2 / max(xyzi(:,3)),...
    sqrt(sum(gradient(xyzi(:,3)).^2)),...
    std(xyzi(:,3)) / xy_dist,...
    (max(xyzi(:,3)) - min(xyzi(:,3))) / xy_dist,...
    (min(xyzi(:,3)) / max(xyzi(:,3))) / xy_dist,...
    (min(xyzi(:,3))^2 / max(xyzi(:,3))) / xy_dist,...
    sqrt(sum(gradient(xyzi(:,3)).^2)) / xy_dist,...
    std(xyzi(:,4)),...
