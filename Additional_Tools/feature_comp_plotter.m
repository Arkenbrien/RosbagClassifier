...
import_export_dir = export_dir + "/import_images";
...
% min_dat_size    =  min([height(gras_data) height(grav_data) height(asph_data) height(foli_data)]);
...
%             scatter(gras_array(1:min_dat_size,x_axis_idx), gras_array(1:min_dat_size,y_axis_idx), 'g*')
            hold on
            scatter(grav_array(1:min_dat_size,x_axis_idx), grav_array(1:min_dat_size,y_axis_idx), 100, 'co', 'Linewidth', 10)
            hold on
            scatter(asph_array(1:min_dat_size,x_axis_idx), asph_array(1:min_dat_size,y_axis_idx), 50, 'k*', 'Linewidth', 10)
            hold on
...
parfor feat_idx = 1:length(labels)
    ...
    hist_fig = figure('Position', fig_size_array, 'DefaultAxesFontSize',14);
