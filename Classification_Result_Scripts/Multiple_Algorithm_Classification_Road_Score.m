...
%                            Rhett Huston
%
%                     FILE CREATION DATE: 02/24/2023
%
%                        Large PCD Scoring
%
% This program loads loads two classification results - one from the road
% surface results and one from the side-of-road results, and stitches them
% together. This is bad don't do this.
% 
%==========================================================================
...
grav_array_in_road = []; chip_array_in_road = []; foli_array_in_road = []; gras_array_in_road = [];
grav_array_off_road = []; chip_array_off_road = []; foli_array_off_road = []; gras_array_off_road = [];
...
%% Load Classification Root File - In Road

root_dir = uigetdir('','Load IN-road classification file');

RESULT_EXPORT_FOLDER = string(root_dir) + "/RESULT_EXPORT";
addpath(RESULT_EXPORT_FOLDER);

AVG_RESULTS_FILE_IN_ROAD = RESULT_EXPORT_FOLDER + "/AVG_RESULTS.mat";

load(AVG_RESULTS_FILE_IN_ROAD);

RESULTS_AVG_IN_ROAD = RESULTS_AVG;

%% Load Classification Root File - Off Road

root_dir = uigetdir('','Load OFF-road classification file');

RESULT_EXPORT_FOLDER = string(root_dir) + "/RESULT_EXPORT";
addpath(RESULT_EXPORT_FOLDER);

AVG_RESULTS_FILE_OFF_ROAD = RESULT_EXPORT_FOLDER + "/AVG_RESULTS.mat";

load(AVG_RESULTS_FILE_OFF_ROAD);

RESULTS_AVG_OFF_ROAD = RESULTS_AVG;
...
%% Loading IN_ROAD data

for clas_idx = 1:1:length(RESULTS_AVG_IN_ROAD.grav(:,1))
    grav_array_in_road = [grav_array_in_road; RESULTS_AVG_IN_ROAD.grav(clas_idx, :)];
end

for clas_idx = 1:1:length(RESULTS_AVG_IN_ROAD.chip(:,1))
    chip_array_in_road = [chip_array_in_road; RESULTS_AVG_IN_ROAD.chip(clas_idx, :)];
end

for clas_idx = 1:1:length(RESULTS_AVG_IN_ROAD.foli(:,1))
    foli_array_in_road = [foli_array_in_road; RESULTS_AVG_IN_ROAD.foli(clas_idx, :)];
end

for clas_idx = 1:1:length(RESULTS_AVG_IN_ROAD.gras(:,1))
    gras_array_in_road = [gras_array_in_road; RESULTS_AVG_IN_ROAD.gras(clas_idx, :)];
end

%% Loading OFF_ROAD data

for clas_idx = 1:1:length(RESULTS_AVG_OFF_ROAD.grav(:,1))
    grav_array_off_road = [grav_array_off_road; RESULTS_AVG_OFF_ROAD.grav(clas_idx, :)];
end

for clas_idx = 1:1:length(RESULTS_AVG_OFF_ROAD.chip(:,1))
    chip_array_off_road = [chip_array_off_road; RESULTS_AVG_OFF_ROAD.chip(clas_idx, :)];
end

for clas_idx = 1:1:length(RESULTS_AVG_OFF_ROAD.foli(:,1))
    foli_array_off_road = [foli_array_off_road; RESULTS_AVG_OFF_ROAD.foli(clas_idx, :)];
end

for clas_idx = 1:1:length(RESULTS_AVG_OFF_ROAD.gras(:,1))
    gras_array_off_road = [gras_array_off_road; RESULTS_AVG_OFF_ROAD.gras(clas_idx, :)];
end

%% Grabbing in-road data
...
if grav_bool
    ...
    for i = 1:length(Manual_Classfied_Areas.grav)
        ...
        grav_in_grav        = inpolygon(grav_array_in_road(:,1), grav_array_in_road(:,2), xy_roi(:,1), xy_roi(:,2));
        chip_in_grav        = inpolygon(chip_array_in_road(:,1), chip_array_in_road(:,2), xy_roi(:,1), xy_roi(:,2));
        foli_in_grav        = inpolygon(foli_array_in_road(:,1), foli_array_in_road(:,2), xy_roi(:,1), xy_roi(:,2));
        gras_in_grav        = inpolygon(gras_array_in_road(:,1), gras_array_in_road(:,2), xy_roi(:,1), xy_roi(:,2));
...
if chip_bool
    ...
    for i = 1:1:length(Manual_Classfied_Areas.chip)
        ...
        % Grabs the indexes for all the points that lie in the polygon
        grav_in_chip        = inpolygon(grav_array_in_road(:,1), grav_array_in_road(:,2), xy_roi(:,1), xy_roi(:,2));
        chip_in_chip        = inpolygon(chip_array_in_road(:,1), chip_array_in_road(:,2), xy_roi(:,1) ,xy_roi(:,2));
        foli_in_chip        = inpolygon(foli_array_in_road(:,1), foli_array_in_road(:,2), xy_roi(:,1), xy_roi(:,2));
        gras_in_chip        = inpolygon(gras_array_in_road(:,1), gras_array_in_road(:,2), xy_roi(:,1), xy_roi(:,2));
...
if foli_bool
    ...
    for i = 1:1:length(Manual_Classfied_Areas.foli)
        ...
        % Grabs the indexes for all the points that lie in the polygon
        grav_in_foli        = inpolygon(grav_array_off_road(:,1), grav_array_off_road(:,2), xy_roi(:,1), xy_roi(:,2));
        chip_in_foli        = inpolygon(chip_array_off_road(:,1), chip_array_off_road(:,2), xy_roi(:,1) ,xy_roi(:,2));
        foli_in_foli        = inpolygon(foli_array_off_road(:,1), foli_array_off_road(:,2), xy_roi(:,1), xy_roi(:,2));
        gras_in_foli        = inpolygon(gras_array_off_road(:,1), gras_array_off_road(:,2), xy_roi(:,1), xy_roi(:,2));
...
if gras_bool
    ...
    for i = 1:1:length(Manual_Classfied_Areas.gras)
        ...
        % Grabs the indexes for all the points that lie in the polygon
		grav_in_gras        = inpolygon(grav_array_off_road(:,1), grav_array_off_road(:,2), xy_roi(:,1), xy_roi(:,2));
        chip_in_gras        = inpolygon(chip_array_off_road(:,1), chip_array_off_road(:,2), xy_roi(:,1) ,xy_roi(:,2));
        foli_in_gras        = inpolygon(foli_array_off_road(:,1), foli_array_off_road(:,2), xy_roi(:,1), xy_roi(:,2));
        gras_in_gras        = inpolygon(gras_array_off_road(:,1), gras_array_off_road(:,2), xy_roi(:,1), xy_roi(:,2));
...
if non_road_bool
    ...
    for i = 1:1:length(Manual_Classfied_Areas.non_road_roi)
        ...
        % Grabs the indexes for all the points that lie in the polygon
		grav_in_nonroad        = inpolygon(grav_array_off_road(:,1), grav_array_off_road(:,2), xy_roi(:,1), xy_roi(:,2));
        chip_in_nonroad        = inpolygon(chip_array_off_road(:,1), chip_array_off_road(:,2), xy_roi(:,1) ,xy_roi(:,2));
        foli_in_nonroad        = inpolygon(foli_array_off_road(:,1), foli_array_off_road(:,2), xy_roi(:,1), xy_roi(:,2));
        gras_in_nonroad        = inpolygon(gras_array_off_road(:,1), gras_array_off_road(:,2), xy_roi(:,1), xy_roi(:,2));
...
plot3(grav_array_in_road(grav_inroad_array,1), grav_array_in_road(grav_inroad_array,2), grav_array_in_road(grav_inroad_array,3), 'c.', 'MarkerSize', 18)
plot3(chip_array_in_road(chip_inroad_array,1), chip_array_in_road(chip_inroad_array,2), chip_array_in_road(chip_inroad_array,3), 'k.', 'MarkerSize', 18)
plot3(foli_array_in_road(foli_inroad_array,1), foli_array_in_road(foli_inroad_array,2), foli_array_in_road(foli_inroad_array,3), 'm.', 'MarkerSize', 18)
plot3(gras_array_in_road(gras_inroad_array,1), gras_array_in_road(gras_inroad_array,2), gras_array_in_road(gras_inroad_array,3), 'g.', 'MarkerSize', 18)

plot3(grav_array_off_road(grav_offroad_array,1), grav_array_off_road(grav_offroad_array,2), grav_array_off_road(grav_offroad_array,3), 'c.', 'MarkerSize', 18)
plot3(chip_array_off_road(chip_offroad_array,1), chip_array_off_road(chip_offroad_array,2), chip_array_off_road(chip_offroad_array,3), 'k.', 'MarkerSize', 18)
plot3(foli_array_off_road(foli_offroad_array,1), foli_array_off_road(foli_offroad_array,2), foli_array_off_road(foli_offroad_array,3), 'm.', 'MarkerSize', 18)
plot3(gras_array_off_road(gras_offroad_array,1), gras_array_off_road(gras_offroad_array,2), gras_array_off_road(gras_offroad_array,3), 'g.', 'MarkerSize', 18)
